...
constructor(options)
{
    ...
    // by default a new entry in the order book will be considered as an update
    this._markNewOrderBookEntriesAsUpdates = true;

    // whether or not library should exit in case an error occurred while calling QueryExchangeState
    this._exitOnQueryExchangeStateError = false;

    // Whether or not order books should be resynced automatically in case a missing Nounce is detected
    this._resyncOrderBooksAutomatically = true;
    ...
    {
        // whether or not library should exit when an error occurs after calling QueryExchangeState
        if (true === options.exitOnQueryExchangeStateError)
        {
            this._exitOnQueryExchangeStateError = true;
        }
        if (false === options.resyncOrderBooksAutomatically)
        {
            this._resyncOrderBooksAutomatically = false;
        }
        // auth
...
_queryExchangeState(pair)
{
    ...
    this._connection.callMethod('QueryExchangeState', [pair], function(d, err){
        ...
        if (null !== err)
        {
            ...
            self.emit('queryExchangeStateError', {pair:pair,type:'error',error:err});
            if (self._exitOnQueryExchangeStateError)
            {
                process.exit(1);
            }
            return;
        ...
        self._decodeData.call(self, d, function(data){
            ...
            if (null === data)
            {
                ...
                // remove subscription
                delete self._subscriptions.markets.pairs[pair];
                self.emit('queryExchangeStateError', {pair:pair,type:'nil_data'});
                if (self._exitOnQueryExchangeStateError)
                {
                    process.exit(1);
                }
...
_processData(data)
{
    try
    {
        ...
        switch (methodName)
        {
            ...
            case 'ub':
            	_.forEach(data.A, (entry) => {
            		this._processUpdateBalanceState(entry);
            	})
            	break;
        };
    }
...
_processUpdateExchangeState(d)
{
    this._decodeData(d, function(data){
        ...
        // ignore update since last nounce is greater
        if (missedCount < 0)
        {
            return;
        }
        if (missedCount > 1)
        {
            if (this._resyncOrderBooksAutomatically)
            {
                if (debug.enabled)
                {
            ...
            if (debug.enabled)
            {
                debug("We missed %d update for market '%s' : last cseq = %d, current cseq", missedCount, data.M, lastCseq, data.N);
            }
        }
        ...
        let orderBookUpdateEvt = {
            ...
            data:{
                buy:_.map(data.Z, entry => {
                    ...
                    {
                        if (!this._markNewOrderBookEntriesAsUpdates)
                        {
                            action = 'add';
                        }
                    }
